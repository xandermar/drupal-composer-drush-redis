...
namespace Drupal\taxonomy_machine_name\Plugin\views\filter;
 ...
 *
 * @ViewsFilter("taxonomy_index_machine_name")
 */
class TaxonomyIndexMachineName extends ManyToOne {
      ...
      if (!empty($this->options['hierarchy']) && $this->options['limit']) {
        ...
        if ($tree) {
          foreach ($tree as $term) {
            $choice = new \stdClass();
            $choice->option = [$term->get('machine_name')->get(0)->value => str_repeat('-', $term->depth) . \Drupal::service('entity.repository')->getTranslationFromContext($term)->label()];
            $options[] = $choice;
          }
        }
      }
  ...
  public function adminSummary() {
    ...
    if ($this->value) {
      ...
      foreach ($terms as $term) {
        $this->valueOptions[$term->get('machine_name')->get(0)->value] = \Drupal::service('entity.repository')->getTranslationFromContext($term)->label();
      }
    }
